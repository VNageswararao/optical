...
class Rack extends CI_Controller {
  public function __construct() {
    ...
    public function ajax_call(){
        $param=$_REQUEST;
        $response=$this->Rack_model->ajax_call($param);
        echo json_encode($response);
 }
	public function index()
	{
      $data['title']='Pharmacy::Rack';
      $data['activecls']='Rack';
      $content=$this->load->view('master/rack',$data,true);
    ...
    public function savedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Rack_model->checkingval($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Rack_model->savedata($data);
	    		if($getresult)
	    		{
	...
	public function editdata()
	{
		...
		if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$getdata=$this->Rack_model->geteditdata($var_array);
	    	if($getdata)
	    	{
	    		$html='';
							                ...
							                <div class="modal-body">
							                    <div class="row">
							                        <div class="col-lg-12 col-md-12" >
							                          <input type="hidden" name="id" id="id" value="'.$data['rack_id'].'">
	...
	public function updatedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Rack_model->editcheck($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Rack_model->updatedata($data,$edit_id);
	    		if($getresult)
	    		{
	...
	public function deletedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Rack_model->deletecheck($var_array);
	    	if($chk_duplication[0]['cnt']==1)
	    	{
	    		$getresult=$this->Rack_model->deletedata($delete_id);
	    		if($getresult)
	    		{
