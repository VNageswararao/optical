...
class Brand extends CI_Controller {
    ...
    public function ajax_call(){
           $param=$_REQUEST;
           $response=$this->Brand_model->ajax_call($param);
           echo json_encode($response);
    }
	public function index()
	{
          $data['title']='Optical::Brand';
          $data['activecls']='Brand';
           $var_array=array($this->session->office_id);
          $last_code_number=$this->db->select('max(code) as last_code_number')
                         ->from('classification')
                         ->where(array('office_id'=>$this->session->office_id,'type'=>2))
          ...
          $data['codeno']=$code;
          $content=$this->load->view('master/brand',$data,true);
    ...
    public function saveclassification()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Brand_model->checkclassification($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data($type,$code,$name,$description,$status);
	    		$getresult=$this->Brand_model->savedata($data);
	    		if($getresult)
	    		{
	...
	public function editclassification()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Brand_model->editcheckclassification($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data($type,$code,$name,$description,$status);
	    		$getresult=$this->Brand_model->updatedata($data,$edit_classificationid);
	    		if($getresult)
	    		{
	...
	public function deleteclassification()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Brand_model->deletecheckclassification($var_array);
	    	if($chk_duplication[0]['cnt']==1)
	    	{
	    		
	    		$getresult=$this->Brand_model->deletedata($delete_classificationid);
	    		if($getresult)
	    		{
