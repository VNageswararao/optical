...
class SuperAdmin extends CI_Controller {
	...
	public function checklogin()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	        ...
	        $result=$this->Super_admin_model->chklogin($utype,$uname,$loginPaswd);
	        $cnt=$result[0]['cnt'];
	        if($cnt>0)
	        {
				$result_data=$this->Super_admin_model->getuserdetails($utype,$uname,$loginPaswd);
				...
				$this->session->set_userdata('user_type', $result_data[0]['user_type']); 
				 ...
				 if($this->input->post("remember")=='1')  
                 {
                  ...
                  } else 
                  {
                    ...
                    $result_data=$this->Super_admin_model->savelog($var_array);
