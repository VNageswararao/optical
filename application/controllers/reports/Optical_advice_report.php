...
class Optical_advice_report extends CI_Controller {
	private $msg;
	private $error;
	...
	public function index()
	{
          ...
          $data['activecls']='Optical_advice_report';
          $content=$this->load->view('reports/optical_advice_report',$data,true);
	  ...
	  public function getopticaladvice()
       {
            ...
            $optica_advice_type='';
             ...
             $optica_advice_date='';
                ...
                if($getresult)
                {
                  $html='<table id="opadv0" class="table table-bordered table-hover"><thead><tr><th>SL NO</th><th>Patient Name</th><th>MRD NO</th><th>Mobile No</th><th>Address</th><th>Optical Advice Date</th><th>Print</th><th>Reason</th><th>Action</th><th>Optical Reason Date</th></tr></thead><tbody>';
                  $sl=1;
                  $exid='';
                  ...
                  //  print_r($getresultdata);exit;
                    if($getresultdata)
                    {
                      ...
                      $optica_advice_date=$getresultdata[0]['optica_advice_date'];
                    }
                     $action='Nill';
                     // if($val!=0)
                     // {
                   ...
                   //  }
                    ...
                    $html.='<tr><td>'.$sl.'</td><td>'.$datr['fname'].' '.$datr['lname'].'</td><td>'.$datr['mrdno'].'</td><td>'.$datr['mobileno'].'</td><td>'.$datr['address'].'</td><td>'.$datr['opticaladvice_date'].'</td><td>'.$print.'</td><td>'.$optica_advice_reason.'</td><td>'.$action.'</td><td>'.$optica_advice_date.'</td></tr>';
                    $sl++;
                  }
