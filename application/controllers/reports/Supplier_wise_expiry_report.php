...
class Supplier_wise_expiry_report extends CI_Controller {
  private $msg;
  private $error;
  ...
  public function __construct() {
        ...
        $this->load->model('Supplierwiseexpiryreport_model');
        $this->load->model('Common_model');
    }
  public function index()
  {
    $data['title']='Pharmacy::Supplier Wise Expiry Report';
    $data['activecls']='Supplier_wise_expiry_report';
    ...
    $content=$this->load->view('reports/supplier_wise_expiry_report',$data,true);
   ...
   public function getdetailed()
    {
      ...
      if($this->form_validation->run() == TRUE)
      {
        ...
        $det_item=trim(htmlentities($this->input->post('det_item')));
         $getsupplierdetails=$this->Supplierwiseexpiryreport_model->getsupplierdetailsmdl($det_fromdate,$det_todate,$det_supplier,$det_modeofpay,$det_item,$this->session->userdata('office_id'));
        ...
        $html='<table class="table table-striped table-bordered dataex-html5-selectors" id="example_det">
                   ...
                   <tbody>';
                       $lt=1;
                        ...
                        {
                           $salesqtysum=$this->Supplierwiseexpiryreport_model->salesdetails($det_fromdate,$det_todate,$dataval['item_id'],$dataval['batchno'],$dataval['expdate'],$dataval['mrp'],$dataval['selling_price'],$this->session->userdata('office_id'));
                           if($salesqtysum)
                           {
                           ...
                           $clrr='';
                           if($dataval['days'] < 0){
                              $clrr="style='background:red;color:#fff;'";
                            }
                            if($dataval['expirydate']){
                             if($dataval['days'] >=0 && $dataval['days'] <=30)
                              $clrr="";
                           }
                             if($dataval['days'] >=0 && $dataval['days'] <=90 || $dataval['days'] < 0){

                              $html.='<tr '.$clrr.'>
                                      <td>'.$lt.'</td>
                                      <td>'.$dataval['supname'].'</td>
                                      <td>'.$dataval['purchase_date'].'</td>
                                      <td>'.$dataval['invoice_no'].'</td>
                                      <td>'.$dataval['itemname'].'</td>
                                      <td>'.$dataval['purqty'].'</td>
