                                ...
                                <div class="card-content">
                                    <div class="card-body">
                                        ...
                                        <ul class="nav nav-tabs">
                                            <li class="nav-item">
                                                <a class="nav-link active" id="base-tab1" data-toggle="tab" aria-controls="tab1" href="#tab1" aria-expanded="true">Add Stock Adjustment</a>
                                            </li>
                                            <li class="nav-item">
...
$( document ).ready(function() {
   table= $('#tableid').DataTable( {
          'processing': true,
          'serverSide': true,
          'ajax':'<?=base_url()?>transaction/Stock_adjustment/ajax_call',
          'columns': [
             { data: 'slno' },
             { data: 'number' },
...
function viewdata(val)
{
    if(val>0)
    {
         ...
         $.ajax({
            type: "POST",
            url: '<?=base_url()?>transaction/Stock_adjustment/viewdata',
            dataType: "json",
...
function savedata()
{
        ...
        $("#overlay").fadeIn(300);
        saveurl="<?php echo base_url(); ?>transaction/Stock_adjustment/savedata";
         $.ajax({
            type: "POST",
            url: saveurl,
            dataType: "json",
      ...
      if(val=='') {
          ...
          Swal.fire({title:"Are you sure?",
            ...
            buttonsStyling:!1}).then((function(t){
              if(t.value)
                {
             delurl="<?php echo base_url(); ?>transaction/Stock_adjustment/deletedata";
                   $.ajax({
            type: "POST",
            url: delurl,
            dataType: "json",
