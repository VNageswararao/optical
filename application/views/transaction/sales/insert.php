                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
											...
											<div class="form-check form-check-inline">
											  ...
											  &nbsp;&nbsp;&nbsp;&nbsp;
											<span id="additem"> <span style="cursor:pointer;display:none" class="notification-tag badge badge-danger float-right m-0" id="addBtnlens"> Add Category</span></span>
											  </label>
											  </div>
...
{
    //alert(product);
	console.log($('#savesales_form').serialize());
     $('#sugession').empty();
    if(product.length<3)
    {
        $('#sugession').parent().hide();
...
function loadautocomplete_framemodel_ajax(product,framemodel)
{
  ...
  $('#addBtnlens').click(function() {
                // Create a new row with two cells
		var tableTrlength=$('#productdetails >tbody >tr').length;
		var itemType = $("#category_id :selected").text();
			$.ajax({
            url:'Sales/addWithoutTableRow',
            type:'post',
			data:{index:tableTrlength,itemtype:itemType},
            success:function(data){
				 $('#productdetails tbody').append(data);
			}	
			});
...
function ProgressSaveEntryData(){
	//alert("hiiii");
	var formData=$('#edit_data_form').serialize();
	console.log(formData);
	 $.ajax({
            url:'Sales/progressSaveEntryData',
            type:'post',
            data:formData,
			success:function(data){
		console.log(data);
