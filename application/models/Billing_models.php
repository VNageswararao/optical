...
// echo $sql;exit;
     ...
     function emorder($requestData)
  {
	...
	$query = "select * from sales_master where sales_master.emergency_order ='1' and office_id<='$office_id' ";
    ...
    $sql = "SELECT sales_id,sales_master.status,name,DATE_FORMAT(sales_date,'%d/%m/%Y') AS sales_date ,invoice_number,total_qty,netamount";
    $sql.=" FROM  sales_master inner join customer on sales_master.customer_id=customer.customer_id where sales_master.office_id=$office_id and sales_master.emergency_order ='1'";
    // getting records as per search parameters
    $isFilterApply=0;
    if( !empty($requestData['search']['value']) ){   //name
      $sql.="  and ( sales_date LIKE '%".$requestData['search']['value']."%' ";
  ...
  function numberTowords($number)
{
 ...
 public function send_to_fitt_update($sales_id,$stt_date,$status,$stt_remarks,$stt_supplier_id)
    {
	//print_r($sales_id);exit;	
   $this->db->query("update sales_master set status=$status,stf_date=$stt_date,stf_supplier_id=$stt_supplier_id where sales_id=$sales_id");
