...
class Category_model extends CI_Model{
 public function __construct()
       ...
       function ajax_call($requestData)
  {
    ...
    $sql.=" FROM classification where office_id=$office_id and type=1";
    ...
    if( !empty($requestData['search']['value']) ){   //name
      ...
      for ($i=0; $i < count($row); $i++) {
	     ...
	     $edit="<button type='button'  onclick=\"editclassification('$classification_id',1,'$code','$name','$description','$status');\" class='btn btn-icon btn-warning mr-1 mb-1'><i class='la la-edit'></i></button>";
      	...
      	$delete='<button onclick="deleteclassification('.$classification_id.',1)" type="button" class="btn btn-icon btn-danger mr-1 mb-1"><i class="la la-trash"></i></button>';

        if(($this->auth->lock_up('item_master',"category_id='$classification_id'")) || ($this->auth->lock_up('supplier',"category_id='$classification_id'")) ||in_array($classification_id,[1,2,3]))
          {
              $delete='<button type="button" class="btn btn-icon btn-primary mr-1 mb-1"><i class="la la-lock"></i></button>';
          }
          if(in_array($classification_id,[1,2,3]))
          {
